...
tf.flags.DEFINE_float("l2_reg_lambda", 2, "L2 regularization lambda (default: 0.0)")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        optimizer = tf.train.AdamOptimizer(5E-4) #学习率
        ...
        def train_step(x_batch, y_batch, ltLoss):
            """
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            with open(os.path.join(out_dir, "result.txt"), 'a+') as f:
                f.write("{}\n".format(result))
        ...
        for batch in batches:
            ...
            train_step(x_batch, y_batch, ltLoss)
            current_step = tf.train.global_step(sess, global_step)
            if current_step % FLAGS.evaluate_every == 0:
                print("Average Loss:", sum(ltLoss)/len(ltLoss))
                ltLoss = []
